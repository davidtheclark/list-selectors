...
function extractSimpleSelectors(selector) {
  ...
  // Make sure not to include 0-9 in order to avoid numbers for e.g. `nth-child`
  var typeSelector = '[^0-9' + COMBINATORS + NEW_BEGINNINGS + '][^' + COMBINATORS + NEW_BEGINNINGS + ']*';
  ...
  return _.map(selector.match(simpleSelectors), stripPseudosFromSimpleSelector);
}
