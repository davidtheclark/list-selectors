...
function extractSimpleSelectors(selector) {
  ...
  // Remove things in parentheses and `:root`
  var prunedSelector = selector.replace(/\(.*\)/, '').replace(/:root/, '');

  return _.map(
    prunedSelector.match(simpleSelectors),
    stripPseudosFromSimpleSelector
  );
}
